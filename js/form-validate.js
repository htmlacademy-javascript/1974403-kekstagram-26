...
pristine.addValidator(
  hashtagsInput,
  (value) => serializeHashtag(value).every((item) => item.startsWith('#') || !item.length),
  'Хэш-тег должен начинаться с символа #',
);

pristine.addValidator(
  hashtagsInput,
  (value) => serializeHashtag(value).every((item) => item.length <= 20 || !item.length),
  'Mаксимальная длина хэш-тега 20 символов (вместе с #)',
);

pristine.addValidator(
  hashtagsInput,
  (value) => serializeHashtag(value).every((item) => /^#[A-Za-zА-Яа-яЁё0-9]{1,19}$/.test(item) || !item.length),
